...
curl -fsSLO "${BASE}/sbe-${VERSION}.xsd"
curl -fsSLO "${BASE}/sbe-${VERSION}.xsd.sha256"
sha256sum -c "sbe-tool-${VERSION}-${TARGET}.sha256" "sbe-${VERSION}.xsd.sha256"

install -m 0755 "sbe-tool-${VERSION}-${TARGET}" /usr/local/bin/sbe-tool
install -D -m 0644 "sbe-${VERSION}.xsd" /usr/local/share/sbe/sbe.xsd
```
...
| Asset | Description |
|---|---|
| `sbe-tool-<version>-<target>` | The executable |
| `sbe-<version>.xsd` | The SBE schema definition from the same upstream commit, for `-Dsbe.validation.xsd` |
| `*.sha256`, `SHA256SUMS` | Checksums for every asset |
...
# Generate C++ codecs from a schema that uses XInclude, validating against the
# XSD shipped with this release
sbe-tool \
  -Dsbe.output.dir=generated \
  -Dsbe.target.language=cpp \
  -Dsbe.xinclude.aware=true \
  -Dsbe.validation.xsd=/usr/local/share/sbe/sbe.xsd \
...
`sbe.validation.xsd` takes a file system path, and the schema definition can
change between SBE versions. Use the `sbe-<version>.xsd` asset from the same
release as the binary; it is copied verbatim from
`sbe-tool/src/main/resources/fpl/sbe.xsd` at the upstream commit that was built.
...
pattern `sbe-tool-<version>-<target>`; the schema definition is published as
`sbe-<version>.xsd`.
...
3. **Publish.** Collect the binaries from all matrix jobs and the
   version-matched XSD, write `SHA256SUMS`, and create the release with `gh release create`, or upload into the existing
...
6. Write the binary, the upstream `sbe.xsd` as `sbe-<version>.xsd`, their
   `.sha256` files, `VERSION` and `COMMIT` to `dist/`.
...
| `-H:IncludeResources=".*\.xsd$"` | Embeds `fpl/sbe.xsd`. SBE itself reads the validation XSD from a file path, so this is retained for parity with the original build command rather than for functionality; the usable copy is the `sbe-<version>.xsd` release asset. |
