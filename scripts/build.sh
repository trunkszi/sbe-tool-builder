...
# Ship the XSD that matches this upstream version so users can validate with
# -Dsbe.validation.xsd=<path>; the property only accepts a file path.
xsd_asset="sbe-${upstream_version}.xsd"
cp "$xsd" "$DIST_DIR/$xsd_asset"
(cd "$DIST_DIR" && sha256sum "$xsd_asset" > "$xsd_asset.sha256")
